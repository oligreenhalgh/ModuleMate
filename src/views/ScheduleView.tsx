...
import { getSchedule, getStats } from '../services/api';
import type { ScheduleEntry, Conflict } from '../services/api';
        ...
        <div className="w-full max-w-[800px] flex flex-col gap-8">
          ...
          <section className="bg-surface p-8 rounded border border-outline-variant/30">
            <div className="flex justify-between items-end mb-6">
              ...
              <div className="text-right">
                <span className="font-mono text-2xl font-bold text-secondary">
                  {stats ? stats.total_credits : '--'}
                  <span className="text-slate-500 font-medium text-lg">/{stats ? stats.required_credits : '--'}</span>
                </span>
            ...
            <div className="relative w-full h-3 bg-surface-high rounded-full overflow-hidden">
              <div
                ...
                style={{ width: stats && stats.required_credits > 0 ? `${Math.min(100, Math.round((stats.total_credits / stats.required_credits) * 100))}%` : '0%' }}
              ></div>
            </div>
            <div className="flex justify-between mt-3">
              <span className="font-mono text-[10px] text-slate-500 uppercase">Core Requirements: {stats && stats.major_required > 0 ? `${Math.round((stats.major_credits / stats.major_required) * 100)}%` : '--'}</span>
              <span className="font-mono text-[10px] text-slate-500 uppercase">Electives: {stats && stats.ue_required > 0 ? `${Math.round((stats.ue_credits / stats.ue_required) * 100)}%` : '--'}</span>
            </div>
          </section>
          ...
          <section className="border border-outline-variant rounded overflow-hidden">
            <div className="bg-surface-low px-6 py-4 flex justify-between items-center border-b border-outline-variant">
              ...
              <span className="font-mono text-xs text-primary bg-primary/10 px-2 py-1 rounded">{totalCredits} Credits Total</span>
            </div>
                    ...
                    <div className="grid grid-cols-12 gap-4 items-center">
                      <div className="col-span-3">
                        <span className={`font-mono text-[10px] uppercase mb-1 block ${hasConflict ? 'text-error' : 'text-slate-500'}`}>
                          {hasConflict ? 'Conflict Warning' : 'Module Code'}
                        </span>
